...
export default function ButtonLink({
  ...
  const bgVariants: Record<string, string> = {
    ...
    tertiary:
      color === "white"
        ? "bg-white hover:bg-gray100 active:bg-gray400 text-[#18191B] border border-[#C4C5CA]"
        : "bg-transparent hover:bg-white hover:bg-opacity-30 active:bg-opacity-30 text-white border border-white",
  };
  ...
  return (
    <Link href={href} passHref legacyBehavior>
      <a
        className={cn(
          "inline-flex items-center justify-center text-heading-05 gap-1.5 font-semibold rounded-full transition-all duration-200",
          bgVariants[variant],
          padding,
          textSize,
          className
